...
for train_index, test_index in kfold.split(Codes,class_):
    ...
    retain=RETAIN(voc_size,50,Codes.shape[1],dropout_value=0.8,gru_units=150)
    retain.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
    
    start= tm.perf_counter()
    retain.fit([Codes[train_index],Demos[train_index]],class_[train_index],
              epochs=epochs,batch_size=batch_size,verbose=0,validation_split=0.1)
    finish=tm.perf_counter()
    print(f"Training time {round(finish-start,3)},second(s)")
    
    start= tm.perf_counter()
    hist=retain.evaluate([Codes[test_index],Demos[test_index]],class_[test_index], batch_size=batch_size)
    finish=tm.perf_counter()
    print(f"Testing time {round(finish-start,3)},second(s)")

    Targets_probas = retain.predict([Codes[test_index],Demos[test_index]]).ravel()
